...
- [x] Selecting a project: disconnects current pane sockets, connects to the new project's panes, updates the UI
- [x] Current project is visually indicated (highlighted or marked) in the list
...
- [x] Visual test: `aiAssert("a command palette overlay is centered on the screen with a search input and a list of projects")`
...
- [x] E2E test: click header project name, dropdown appears, select project, terminals switch
- [x] Visual test: `aiAssert("a top header bar shows the project name, with a dropdown list of other projects open below it")`
...
- [x] `GET /api/tmux/sessions` returns all tmux sessions NOT already mapped to a DanCode project
- [x] "New Project" form has an "Adopt existing tmux session" toggle/option
- [x] When enabled, a dropdown lists available orphaned tmux sessions
...
- [x] E2E test: create a tmux session manually, open New Project form, adopt it, see its panes in DanCode
