...
- [ ] Backend creates a tmux session (`dancode-test`) on startup if it doesn't exist
...
- [ ] Page background, font, and spacing match the polished/modern design intent (subtle borders, not raw/unstyled)
- [ ] Unit test: Socket.io connection lifecycle (connect, receive data, disconnect)
...
- [ ] On first server start, if `~/.dancode/auth-token` does not exist, generate a cryptographically random token and write it to the file
- [ ] Token is logged to server console on first generation so the user can copy it
...
- [ ] `POST /api/projects` accepts `{ name, path }`, validates inputs, creates config in `~/.dancode/`
- [ ] If the directory at `path` does not exist, create it
- [ ] Creates tmux session `dancode-<slug>` with two panes: pane 0 runs `cd <path>`, pane 1 runs `cd <path> && claude --dangerously-skip-permissions`
...
- [ ] Desktop (>768px): two panes display side by side in a 50/50 split by default
- [ ] If a 3rd pane is enabled: panes display in a 33/33/33 split
- [ ] Each pane has a label showing its type (CLI, Claude, Ralph)
- [ ] Users can click a pane to focus it for keyboard input
