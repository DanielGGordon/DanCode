...
- `App` — Root component, checks localStorage for auth token; shows `LoginScreen` if absent, otherwise renders the header with "New Project" button, and either the `NewProjectForm`, `PaneLayout` (when a project is selected), or default `Terminal` view. Ctrl+K opens the command palette for project switching.
