...
- `FileViewer` — Phase 6 CodeMirror 6 editor pane. Loads the per-extension language pack via `editor/language.js`, exposes find/replace (Ctrl+F / Ctrl+H), undo/redo (Ctrl+Z / Ctrl+Y / Ctrl+Shift+Z), multi-cursor (Alt+Click, Ctrl+D), and an always-on line-number gutter from CM 6's standard extensions. Fetches via `GET /api/projects/:slug/files/*` and saves via `PUT /api/projects/:slug/files/*` on both Ctrl+S and editor blur (only when dirty). Unknown extensions render in plain text. Props: `token`, `slug`, `filePath`, `focused`, `onFocus`. Also exports `buildFileUrl(slug, path)`.
