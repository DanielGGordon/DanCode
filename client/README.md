...
- `Terminal` — xterm.js terminal with forwardRef. Connects via Socket.io `/terminal/{uuid}`. Solarized Dark theme, auto-resize, reconnection UX, drag-and-drop upload. Supports `readFirst` prop (no auto-focus), pinch-to-zoom on mobile, per-tab keyboard zoom (Phase 2: Ctrl/Cmd + =, -, 0 — bounded [8,32]px, default 13, persisted per terminal id in `localStorage` under `dancode-zoom-terminal:<id>`, with FitAddon `.fit()` + a follow-up `resize` socket emit so the running shell sees the new geometry), and imperative API via ref (`sendInput`, `focus`, `setFontSize`, `getFontSize`). Props: `token`, `terminalId`, `projectSlug`, `focused`, `readFirst`, `onFocus`, `onConnectionStateChange`.
