...
│   │   ├── build.gradle.kts    # Compose enabled, Compose BOM 2024.09.00, compiler 1.5.14; depends on :terminal-view (Phase 1); `testReleaseUnitTest` disabled because ui-test-manifest is debug-only
│   │   └── src/
│   │       ├── main/
│   │       │   ├── AndroidManifest.xml
│   │       │   ├── res/values/themes.xml
│   │       │   └── java/com/dancode/android/
│   │       │       ├── MainActivity.kt          # ComponentActivity that hosts the Compose tree
│   │       │       └── ui/HomeScreen.kt         # Phase-0 placeholder composable; exposes HomeScreenTags.PLACEHOLDER for tests
│   │       └── test/java/com/dancode/android/
│   │           ├── SmokeTest.kt                 # JUnit trivial unit test (2+2=4) — proves test path works headless
│   │           └── HomeScreenRenderTest.kt     # Robolectric @Config(sdk=33) Compose render test; mounts HomeScreen() and asserts the testTag + "DanCode" text
