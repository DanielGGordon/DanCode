...
│   │       │       ├── MainActivity.kt          # Phase 2/5: ComponentActivity sets up TokenStorage, OkHttp clients (auth + bearer-injected), AuthApi/ProjectsApi/TerminalsApi, TerminalFontSizeStore; AppNav composable observes AppNavController.state and renders Login / Dashboard / TerminalList / TerminalSwiper(TerminalHost) for each screen; routes onUnauthorized → tokenStorage.clear() + nav.unauthorized(); routes onSignOut → tokenStorage.clear() + nav.signOut(); Retry buttons drive controller.load() in a rememberCoroutineScope
│   │       │       ├── AppNavController.kt       # Phase 5: headless three-level nav stack (Login / Dashboard / TerminalList / Terminal with sibling list). MutableStateFlow<Screen>; navigateTo/back; signOut clears any pending destination + routes to Login; unauthorized() captures the *first* current screen as pending so onLoggedIn() resumes there after re-login. Subsequent 401s never overwrite the pending destination with Screen.Login
│   │       │       ├── auth/
...
│   │       │       └── terminal/                # Phase 3+4: live terminal stack — REST, socket, emulator binding, full-screen view, raw-mode input, scroll routing
...
│   │       │           ├── TerminalListScreen.kt        # LazyColumn over terminals; onSelect → TerminalScreen via MainActivity. Phase 5: header Back row routes nav.back() to dashboard; Error pane renders a Retry button keyed to controller.load()
│   │       │           ├── TerminalFontSizeStore.kt     # Phase 5: per-terminal-id font-size persistence via SharedPreferences (file `dancode-terminal-font-prefs`). read/save/reset/step(+/-) clamps to [MIN=8, MAX=40] in steps of STEP=2; DEFAULT=14
│   │       │           ├── PinchZoomDetector.kt          # Phase 5: pure-logic pinch detector. onScale(scale) multiplies a running cumulative; emits FontSizeAction.Increase / Decrease + resets when cumulative crosses 1.20× / 0.83×; reverses direction → reset; tiny (<1.001×) scales ignored
│   │       │           ├── TerminalSwiper.kt             # Phase 5: HorizontalPager wrapper that swipes between a project's sibling terminals. Caller passes a per-page Composable slot so production wires it to TerminalHost and tests can stub it
│   │       │           ├── TerminalInputEncoder.kt      # cooked-mode: strips trailing \r/\n, appends "\r"
│   │       │           ├── ControlKey.kt                # Phase 4: enum with label + golden byte sequence for Esc / arrows / Enter / Ctrl+C / Tab / Shift+Tab — the key-bar uses this directly, no hand-rolled strings anywhere
│   │       │           ├── InputModePolicy.kt           # Phase 4: resolve(EmulatorModeState) → InputMode (Cooked/Raw). Auto: Raw whenever alt-screen OR mouse-tracking is on. Manual setManualOverride wins until cleared.
│   │       │           ├── MouseWheelEncoder.kt         # Phase 4: SGR (1006) — wheelUp/wheelDown(col,row) → "ESC[<64;c;rM"/"ESC[<65;c;rM"; coords clamp to 1 for events landing before the grid origin is known
│   │       │           ├── ScrollRouter.kt              # Phase 4: onTwoFingerDrag(deltaRows,col,row,state) → SendBytes(SGR wheel sequence × |delta|) when raw mode, LocalScroll(delta) otherwise, None when delta=0
...
│   │       │           ├── TerminalConnection.kt        # State machine Idle→Connecting→Connected→Reconnecting→Disconnected. On *re*connect emits sink.reset() before output; sendResize buffers pre-Connect calls + replays on connect; auto-redials transport.connect on disconnect; stop() suppresses further reconnects. Phase 4: sendRaw(data) forwards verbatim bytes (no cooked CR) when Connected
...
│   │       │           ├── TerminalScreen.kt            # Compose: Back + horizontally-scrolling header (label, font-size A-/A/A+ row, override-mode Auto→Cooked→Raw toggle) + AndroidView slot + control key bar (44dp tap targets, horizontal scroll) + cooked-mode OutlinedTextField (disabled when inputMode=Raw) + Send + "Reconnecting…" overlay (state == Reconnecting). Tag-helpers: TerminalScreenTags.keyTag(ControlKey). FontSizeAction enum (Increase/Decrease/Reset) emitted from header buttons and from the pinch detector
│   │       │           └── TerminalHost.kt              # Wires SocketIoTransport + RemoteTerminalSession + TerminalEmulatorSink + TerminalConnection to com.termux.view.TerminalView; main-thread sink marshalling; AndroidView update() emits resize from renderer fontWidth/fontLineSpacing. Phase 4: LaunchedEffect polls emulator alt-screen + mouse-tracking flags at 60Hz into Compose state; InputModePolicy resolves the active mode; pointerInput on the view's parent Box detects two-finger drags → ScrollRouter → connection.sendRaw (raw) or fall-through (local). Phase 5: reads/writes TerminalFontSizeStore keyed by terminal.id, drives TerminalView.setTextSize on changes (re-emits resize on the next layout pass); detectTransformGestures pointerInput feeds PinchZoomDetector → applyFontAction
...
│   │           ├── AppNavControllerTest.kt        # Phase 5: 13 specs — initial Screen (Login when no token / Dashboard with token); navigateTo; hierarchical back; unauthorized() captures current screen, onLoggedIn() resumes it; multiple unauthorized() calls keep first pending; signOut() wipes pending; onLoggedIn() defaults to Dashboard with no pending
│   │           ├── ConfigurationChangeTest.kt     # Phase 5: Robolectric — AndroidManifest activity-info declares ORIENTATION|SCREEN_SIZE|SCREEN_LAYOUT|KEYBOARD_HIDDEN configChanges; TerminalViewMetrics yields different grids for portrait vs landscape and for keyboard-shown vs keyboard-hidden viewports; TerminalConnection emits a fresh resize per configuration change in order
│   │           └── terminal/                # Phase 3+4: live-terminal tests (all headless — no device, no live server)
...
│   │               ├── TerminalScreenTest.kt         # Overlay surfaces only when state == Reconnecting; typing + Send forwards typed line to onSend; Back invokes onBack; Phase 4: key bar renders every ControlKey (scrolls horizontally so performScrollTo before assertIsDisplayed), taps dispatch the right enum, buttons disabled when not Connected, cooked input disabled in Raw mode, override toggle cycles Auto→Cooked→Raw→Auto via mutableStateOf
│   │               ├── TerminalConnectionTest.kt     # Drives FakeTransport+FakeSink: start→Connecting; onConnected→Connected; output→sink.write; first connect → no reset (initial replay kept); disconnect→Reconnecting + reconnect→reset before next output; multiple cycles each reset once; auto-redial via transport.connect; stop suppresses; sendLine encodes via TerminalInputEncoder; sendResize buffered pre-Connect, replayed on Connect; Phase 4: sendRaw forwards verbatim, no-ops before Connected
...
│   │               ├── ControlKeyTest.kt             # Phase 4: golden byte sequence per ControlKey (Esc, four arrows, Enter, Ctrl+C, Tab, Shift+Tab); labels unique + non-blank
│   │               ├── InputModePolicyTest.kt        # Phase 4: defaults Cooked; engages Raw on alt-screen or mouse-tracking; reverts when both clear; manual override wins; clear returns to auto
│   │               ├── MouseWheelEncoderTest.kt      # Phase 4: ESC[<64;c;rM (up) and ESC[<65;c;rM (down); coordinate pass-through; 0/-N clamps to 1
│   │               ├── ScrollRouterTest.kt           # Phase 4: alt-on → SendBytes(SGR sequence × |delta|), mouse-tracking-on outside alt → SendBytes too, normal → LocalScroll(delta), 0 → None; routing keyed on state passed at gesture time
│   │               ├── AltScreenRawModeTransitionTest.kt  # Phase 4 golden: drives the real Termux TerminalEmulator with chunked DECSET 1049 / 1006 / 1000 stream, snapshots isAlternateBufferActive + isMouseTrackingActive after each chunk, asserts the full Cooked→Raw→Raw→Raw→Cooked timeline matches InputModePolicy.resolve()
│   │               ├── TerminalFontSizeStoreTest.kt # Phase 5: read default + round-trip; values are per-id; survives store recreation; save clamps below MIN / above MAX; step persists; step decrement clamps at MIN; reset returns to DEFAULT
│   │               ├── PinchZoomDetectorTest.kt    # Phase 5: small scales emit nothing; one big pinch past 1.20× / 0.83× emits Increase / Decrease; small pinches accumulate; emit-then-reset; reset clears; direction reversal resets the accumulator
│   │               └── TerminalSwiperTest.kt       # Phase 5: HorizontalPager renders initial terminal at initialIndex; swipeLeft advances; swipeRight retreats; single-terminal list renders without crashing
