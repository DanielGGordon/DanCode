...
│   ├── settings.gradle.kts     # Registers :app; later phases add :terminal-emulator + :terminal-view
...
│       ├── build.gradle.kts    # Compose enabled, Compose BOM 2024.09.00, compiler 1.5.14; `testReleaseUnitTest` disabled because ui-test-manifest is debug-only
│       └── src/
│           ├── main/
│           │   ├── AndroidManifest.xml
