...
│   ├── scripts/bootstrap-toolchain.sh # Provisions project-local JDK 17 (Temurin 17.0.12+7) + Android SDK (platforms;android-34, build-tools;34.0.0) under android/.toolchain/; accepts SDK licenses non-interactively; idempotent; never touches system Java
│   ├── .toolchain/             # Bootstrapped JDK + SDK + env.sh (gitignored; generated by scripts/bootstrap-toolchain.sh)
│   ├── gradlew                 # Standard Gradle 8.9 wrapper, customised at top to source .toolchain/env.sh so each invocation uses the project-local JDK 17 only for that process
...
│   ├── build.gradle.kts        # Top-level plugin declarations (AGP 8.5.2, Kotlin 1.9.24)
...
│   └── app/                    # :app module — application id com.dancode.android, minSdk 30, targetSdk 34
